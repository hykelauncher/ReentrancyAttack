from brownie import accounts, config, chain, network, EtherStore, Attack
...
def print_balances(msg):
    ...
    att_bal = Web3.fromWei(Attack[-1].balance(), "ether")
    ...
    print(f"{msg} :Attack Balance is {att_bal} ether")
...
def main():
    ...
    print(f"Contract deployed at {e}")

    a = Attack.deploy(EtherStore[-1].address, {"from": account})
    print(f"Contract deployed at {a}")
